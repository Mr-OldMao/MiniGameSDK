...
namespace AlipaySdk
{
     ...
     * APAssetBundle.load 触发下载后，这里并不会获取到真正的 Asset Bundle 数据
     */
    public class DownloadHandlerAPAssetBundle : DownloadHandlerScript
    {
        ...
        public DownloadHandlerAPAssetBundle(string uri, uint crc)
        {
            _uri = uri;
            _crc = crc;
        }
            ...
            get
            {
                if (_assetBundle == null)
                {
                    if (APAssetBundle.isAbfsReady)
                    {
                        if (_contentLength != 0)
                        {
                            Debug.LogError($"DownloadHandlerAPAssetBundle contentLength not 0!");
                            return null;
                        }
                        _assetBundle = AssetBundle.LoadFromFile(_uri, _crc);
                        APAssetBundle.bundle2path.Add(_assetBundle, _uri);
                    }
                    else
                    {
        ...
        protected override bool ReceiveData(byte[] data, int dataLength)
        {
            ...
            if (!_isFallbackNoticed)
            {
                ...
                Debug.LogWarning("APAssetBundle 仅在 WebGL 方案有优化效果，当前环境下回滚到 UnityWebRequestAssetBundle 加载实现。");
            }
#endif
